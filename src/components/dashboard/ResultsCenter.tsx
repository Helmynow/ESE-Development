...
export function ResultsCenter({ user }: ResultsCenterProps) {
  ...
  const employeeResults = allStaff.map(employee => {
    ...
    const allScores = Object.keys(ratings[0]?.scores || {})
    ...
    const averageScore = weightedScores.length > 0 
    ...
    return {
      ...
      progress: Math.min(100, (ratings.length / 5) * 100), // Assuming 5 expected evaluators
