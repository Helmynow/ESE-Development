...
const cloneTasks = (taskList: AutomatedTask[] = []) =>
  taskList.map(task => ({ ...task, recipients: [...task.recipients] }))

const calculateNextRun = (frequency: AutomatedTask['frequency'], referenceDate = new Date()) => {
...
export function AutomationCenter({ user }: AutomationCenterProps) {
  const [tasks, setTasks] = useKV<AutomatedTask[]>('automated-tasks', mockTasks)
  ...
  const handleToggleTask = (taskId: string) => {
    ...
    const previousTasks = cloneTasks(currentTasks)
    const updatedTasks = currentTasks.map(task => {
      if (task.id !== taskId) {
        return task
      }

      const toggledActive = !task.isActive
      return {
        ...task,
        isActive: toggledActive,
        nextRun: toggledActive ? calculateNextRun(task.frequency) : task.nextRun
      }
    })
    ...
    if (!targetTask) {
      toast.error('Unable to locate the selected task')
      return
    }

    const result = setTasks(updatedTasks)
    toast.success(`Task ${targetTask.isActive ? 'activated' : 'paused'}: ${targetTask.name}`)

    void Promise.resolve(result).catch(error => {
      console.error('Failed to update task status in KV store:', error)
      toast.error('Failed to persist task status change. Reverting to previous state.')
  ...
  const handleRunNow = (taskId: string) => {
    ...
    const updatedTasks = currentTasks.map(task => {
      ...
      return {
        ...
        nextRun: calculateNextRun(task.frequency, now)
      }
    })
    ...
    const result = setTasks(updatedTasks)
    toast.success(`Executing task: ${targetTask.name}`)

    void Promise.resolve(result).catch(error => {
      console.error('Failed to record manual task execution in KV store:', error)
      toast.error('Failed to persist execution metadata. Reverting to previous state.')
      setTasks(previousTasks)
    })
  }
  ...
  const handleDeleteTask = (taskId: string) => {
    ...
    const result = setTasks(updatedTasks)
    toast.success(`Deleted task: ${targetTask.name}`)

    void Promise.resolve(result).catch(error => {
      console.error('Failed to delete task in KV store:', error)
      toast.error('Failed to persist task deletion. Reverting to previous state.')
      setTasks(previousTasks)
    })
  }
  ...
  const handleCreateTask = () => {
    ...
    const trimmedSchedule = newTask.schedule.trim()

    if (!trimmedName || !trimmedDescription || !trimmedSchedule) {
      toast.error('Please complete all required fields before creating a task')
      return
    }
    ...
    const previousTasks = cloneTasks(currentTasks)
    const now = new Date()
    const createdTask: AutomatedTask = {
      id: generateTaskId(),
      name: trimmedName,
      ...
      isActive: true,
      lastRun: now.toISOString(),
      nextRun: calculateNextRun(newTask.frequency, now),
      recipients: newTask.recipients.length > 0 ? [...newTask.recipients] : [user.department],
      conditions: newTask.conditions.trim()
    }

    const updatedTasks = [...currentTasks, createdTask]
    const result = setTasks(updatedTasks)

    toast.success(`Created new automated task: ${createdTask.name}`)
    setIsCreating(false)
    setNewTask({
      name: '',
      description: '',
    ...
    void Promise.resolve(result).catch(error => {
      console.error('Failed to create task in KV store:', error)
      toast.error('Failed to persist new task. Reverting to previous state.')
      setTasks(previousTasks)
    })
  }
