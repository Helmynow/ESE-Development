...
import { getAllStaff, type StaffMember } from '@/data/staff-data'

const staffDirectory = getAllStaff()

const resolveStaffDetails = (staffId: string) => {
  const staffMember = staffDirectory.find(member => member.id === staffId)

  return {
    staffMember,
    name: staffMember?.name ?? 'Unknown employee',
...
const mockNominations: Nomination[] = [
  (() => {
    const { staffMember, name, department } = resolveStaffDetails('eng-teacher-1')
    return {
      id: '1',
      nomineeId: staffMember?.id ?? 'eng-teacher-1',
      nomineeName: name,
  ...
  (() => {
    const { staffMember, name, department } = resolveStaffDetails('admin-manager')
    return {
      id: '2',
      nomineeId: staffMember?.id ?? 'admin-manager',
      nomineeName: name,
      nomineeDepartment: department,
      category: 'innovation',
  ...
  (() => {
    const { staffMember, name, department } = resolveStaffDetails('math-teacher-1')
    return {
      id: '3',
      nomineeId: staffMember?.id ?? 'math-teacher-1',
      nomineeName: name,
      nomineeDepartment: department,
      category: 'risingstar',
...
export function RecognitionCenter({ user }: RecognitionCenterProps) {
  ...
  const staffMembers = useMemo(() => getAllStaff(), [])

  const staffLookup = useMemo(() => {
    const lookup = new Map<string, StaffMember>()
  ...
  const selectedStaff = nominationForm.nomineeId
    ? staffLookup.get(nominationForm.nomineeId)
    : undefined

  const normalizedNominations = useMemo(() => {
    if (!nominations) {
      return [] as Nomination[]
    }

    return nominations.map(nomination => {
      const staffMember = staffLookup.get(nomination.nomineeId)

      if (staffMember) {
        if (
          nomination.nomineeName !== staffMember.name ||
          nomination.nomineeDepartment !== staffMember.department
        ) {
          return {
            ...nomination,
            nomineeName: staffMember.name,
            nomineeDepartment: staffMember.department
          }
        }
      ...
      return {
        ...nomination,
        nomineeName: nomination.nomineeName || 'Unknown employee',
        nomineeDepartment: nomination.nomineeDepartment || 'Unknown department'
      }
    })
  }, [nominations, staffLookup])
  ...
  useEffect(() => {
    ...
    const migrated = nominations.map(nomination => {
      const staffMember = staffLookup.get(nomination.nomineeId)
      ...
      if (!nomination.nomineeName || !nomination.nomineeDepartment) {
        hasChanges = true
        return {
          ...nomination,
          nomineeName: nomination.nomineeName || 'Unknown employee',
    ...
    if (hasChanges) {
      setNominations(migrated)
    }
  }, [nominations, setNominations, staffLookup])
  ...
  const handleSubmitNomination = async () => {
    if (!nominationForm.nomineeId || !nominationForm.category || !nominationForm.description) {
      toast.error('Please fill in all required fields')
      return
    }

    const staffMember = staffLookup.get(nominationForm.nomineeId)

    const nomineeName = staffMember?.name || 'Unknown employee'
    const nomineeDepartment = staffMember?.department || 'Unknown department'

    const newNomination: Nomination = {
      id: `${Date.now()}`,
      nomineeId: nominationForm.nomineeId,
      nomineeName,
    ...
    setNominations(prev => {
      ...
      return [...existing, newNomination]
    })

    toast.success(`Nomination submitted for ${nomineeName}`)
  ...
  const generateAISuggestions = async () => {
    ...
    try {
      const activeNominations = normalizedNominations.filter(n => n.status === 'voting' || n.status === 'pending').length
      const departmentCounts = normalizedNominations.reduce((acc, nom) => {
    ...
    <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    >
                      <span className="flex flex-col items-start text-left">
                        <span className="text-sm font-medium">
                          {selectedStaff ? selectedStaff.name : 'Select an employee'}
                        </span>
                        <span className="text-xs text-muted-foreground">
                          {selectedStaff ? selectedStaff.department : 'Search by name or department'}
                        </span>
                      </span>
                            ...
                            >
                              <Check
                                className={`mr-2 h-4 w-4 ${
                                  nominationForm.nomineeId === option.id ? 'opacity-100' : 'opacity-0'
                                }`}
                              />
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="Select a nominee to view department"
                  readOnly
                  disabled
                />
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{normalizedNominations.length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {normalizedNominations.filter(n => n.status === 'voting').length}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {categories.map(category => {
          ...
          const categoryNominations = normalizedNominations.filter(n => n.category === category.id)
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                          ...
                          <div className="flex gap-2">
                            {isUnknownStaff && (
                              <Badge variant="destructive">Unknown employee</Badge>
                            )}
