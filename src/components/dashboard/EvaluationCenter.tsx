...
export function EvaluationCenter({ user }: EvaluationCenterProps) {
  ...
  const userEvaluations: Evaluation[] = authorizedEvaluations.map(assignment => {
    ...
    return {
      ...
      totalRaters: getTotalRatersForEmployee(assignment.evalueeId),
  ...
  // Helper function to get total expected raters for an employee
  function getTotalRatersForEmployee(employeeId: string): number {
    const employee = allStaff.find(s => s.id === employeeId)
    if (!employee) return 1
    
    // Count expected evaluators: self + supervisors + CEO + P&C Head + limited peers
    let count = 1 // self
    
    // Add supervisors based on role
    if (employee.role === 'staff') {
      if (employee.department.includes('Teacher')) count += 1 // Principal/Coordinator
      if (employee.department.includes('Assistant') || employee.department.includes('Nurse')) count += 1 // Admin Manager
    }
    
    count += 2 // CEO + P&C Head
    
    // Add limited peer evaluations for staff
    if (employee.role === 'staff') {
      count += Math.min(2, allStaff.filter(s => 
        s.role === 'staff' && 
        s.id !== employeeId && 
        s.department.split(' ')[0] === employee.department.split(' ')[0]
      ).length)
    }
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 text-sm text-muted-foreground mb-4">
                          ...
                          <div>
                            <span className="font-medium">Weight:</span><br className="sm:hidden" /> {roleAndWeight?.weight || 0}%
                          </div>
                          <div className="flex items-center gap-2">
